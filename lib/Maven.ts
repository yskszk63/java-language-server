...
export function dependencies(options: Options, callback: (errors: Error[], result?: Result) => void) {
    ...
    var dependencies: { [id: string]: Dependency } = {};
    var errors = [];
    ...
    function go(callback) {
        if (!options.dependencies) {
            return callback(new Error("options.dependencies is required"));
        }

        if (!(options.dependencies instanceof Array)) {
            return callback(new Error("options.dependencies must be an array."));
        }

        return options.dependencies.forEach(function(d) {
